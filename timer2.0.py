...
def countdown(countdown_frame, reminder_text, countdown_text, reminder_time):
    ...
    def update_and_schedule(remaining_time):
        ...
        if reminder_time >= 0:
            ...
            countdown_frame.after(1000, update_and_schedule, reminder_time)
            # Check if one minute has passed and update the remaining time
            if reminder_time % 60 == 0:
                reminder_datetime = datetime.datetime.now() + datetime.timedelta(seconds=reminder_time)
...
def set_reminder():
    ...
    try:
        ...
        if time_difference > 0:
            ...
            countdown(countdown_frame, reminder_text, countdown_text, int(time_difference))
...
def resume_countdown(event):
    ...
    try:
        ...
        if remaining_time > 0:
            ...
            countdown_frame = tk.Frame(root)
            countdown_frame.pack(pady=10)
            countdown(countdown_frame, reminder_text, countdown_text, remaining_time)
        else:
